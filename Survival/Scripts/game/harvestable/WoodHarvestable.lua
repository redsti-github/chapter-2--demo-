...
function WoodHarvestable.client_onMelee( self, hitPos, attacker, damage )
	if type( attacker ) == "Player" then
...
function WoodHarvestable.server_onFixedUpdate( self, deltaTime )

	local data = self.harvestable:getPublicData()

	if (data ~= nil) then
		self:sv_onHit( data[1], data[2])
	end
